...
async def test_project_integrations_config(client, auth_headers):
    ...
    assert (await client.get(f"/projects/{pid}/integrations-config", headers=auth_headers)).json()[
        "integration_ids"
    ] == []
...
async def test_project_oauth_integrations_config(client, auth_headers):
    ...
    assert (
        await client.post(f"/projects/{pid}/oauth-integrations-config/google-drive", headers=auth_headers)
    ).status_code == 201
    body = (await client.get(f"/projects/{pid}/integrations-config", headers=auth_headers)).json()
    assert body["integration_keys"] == ["google-drive"]

    assert (
        await client.delete(f"/projects/{pid}/oauth-integrations-config/google-drive", headers=auth_headers)
    ).status_code == 204
    assert (await client.get(f"/projects/{pid}/integrations-config", headers=auth_headers)).json()[
        "integration_keys"
    ] == []
